...
function validate() {
    ...
    if (inputCode.length <= 0) {
        ...
        createCode();
        document.getElementById('identCode').value = '';
        codeSuces.innerHTML = '验证码输入错误，请重新输入。';
        // codeSuces.style.color = "#FF3E4B";
        statu_k = 0;
...
$(document).ready(function () {
    ...
    function CountCho() {
        $('.toSerBtn').bind('click', function () {
            ...
            if (s.length == 0) {
                alert('请勾选跨国工厂营运所在地，若无符合地区请勾选「其他」；若无跨国工厂营运请点击「X」。');
                s = [];
                str = '';
                $('.choNySpan').empty();
            } else {
                $('.transnat').hide();
                str = s.join(',');
                $('.choNySpan').append('已选择:' + '<span>' + str + '</span>');
            }
            ...
            success: function (data) {
                ...
                if (chenkInfo[0] == 'N' || chenkInfo[0] == 'undefined' || chenkInfo[0] == '') {
                    ...
                    if (SourLoadLink == '' || data.farm == 'N') {
                        $('.thxparg').html('');
                        $('.thxparg').html('您所索取的资料，<br />将在3个工作天内寄发至您指定的 Email');
                        $('.thxSourLoad1').hide();
                        $('#goIndex').removeClass('thxSourLoad2');
                    } else if (data.farm == 'Y' && SourLoadLink != '') {
                        $('.thxparg').html('');
                        $('.thxparg').html('我们会尽快处理您的 e化需求');
                        $('.thxSourLoad1').show();
        ...
        if ($('#agree').is(':checked')) {
            if (statu_k == 1) {
                ...
                if (!emailError && !contactNotFinish && !demandNotFinish && !consContNotFinish && !tranNotFinish) {
                    ...
                    var txt = '尚有联络资料未填写或Email格式错误，请重新确认。';
