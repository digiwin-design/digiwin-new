...
$(function(){
	...
	if(URL_fileName !== pageIndex){
		...
		if(URL_hash !== ''){
			$htmlBody.stop().animate({
				scrollTop: Math.ceil($(URL_hash).offset().top - _navLinkOuterHeight)
			})
		}
	...
	/*點擊連到專家區塊*/
	if(URL_hash == "#expert"){
		var _expertTop = $('#expert').offset().top;
		$htmlBody.stop().animate({scrollTop : Math.ceil(_expertTop) - 100});
		
		$('.expertBlock').click(function(){
			$htmlBody.stop().animate({scrollTop : Math.ceil(_expertTop) - 100});
		})
	}
		...
		/*點擊法規衝擊*/
	if(URL_hash == "#effect"){
		var _effectTop = $('#effect').offset().top;
		$htmlBody.stop().animate({scrollTop : Math.ceil(_effectTop) - 100});
		
		$('.effectBlock').click(function(){
			$htmlBody.stop().animate({scrollTop : Math.ceil(_effectTop) - 100});
		})
		// console.log(_effectTop)
	}
